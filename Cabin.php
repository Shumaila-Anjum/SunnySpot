...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
   ...
   // $imageURL = $defaultImage; // Default image
    ...
    if (isset($_GET["id"])) {
        $cabinid = $_GET["id"];
         // Check if no new image is uploaded, retain the original image // NEW CODE
         if ($imageURL === null) {
            $imageQuery = $conn->query("SELECT Cabin_Image FROM cabin WHERE CabinID = $cabinid");
            if ($imageQuery && $imageQuery->num_rows > 0) {
                $existingImage = $imageQuery->fetch_assoc();
                $imageURL = $existingImage['Cabin_Image']; // Retain original image
            }
        }
        ...
        // For new cabin insertion
        if ($imageURL === null) {    // Ensure default image is used if none provided 
            ...
            <form method="POST" enctype="multipart/form-data" onsubmit="return checkPricesBeforeSubmit()">
                ...
                <input type="text" id="CabinName" name="CabinName" value="<?php if (isset($_GET['id']))
                    ...
                    <div class="input-with-prefix">
                        ...
                        <input type="number" id="Price_Per_Night" name="Price_Per_Night" min="1" step="1" oninput="validatePrices()"
                            value="<?php if (isset($_GET['id'])) echo $data['Price_Per_Night'] ?>" required>
                    </div>
